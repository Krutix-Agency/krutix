...
function startPageTypewriters() {
  const targets = [
    { id: 'typed-shipped', text: 'shipped.' },
    { id: 'typed-about', text: 'of digital systems.' },
    { id: 'typed-careers', text: 'that matter.' }
  ];
  ...
  targets.forEach(target => {
    ...
    function tick() {
      el.textContent = text.slice(0, ++ci) + (ci < text.length ? '|' : '');
      if (ci < text.length) setTimeout(tick, 80);
    }
    setTimeout(tick, 500);
  });
}
...
(function () {
  ...
  const pts = Array.from({ length: 100 }, () => ({
    ...
    r: Math.random() * 1.5 + .5, o: Math.random() * .6 + .1
  }));
